...
Chart::Chart(int specH)
{
        spect = QImage(specH,specH/2,QImage::Format::Format_RGB32);
}
...
void Chart::drawLinearGrid(QPainter &painter, QRect geometry)
{
    ...
    for(int py=0; py<=gridNumY; py++){
        ...
        painter.drawText(QPointF(gx+gw+40-(font.pointSize()*4), gy+(font.pointSize()/2)+py*dy), QString().sprintf("%4.1f",(1.0-py*0.1)));
    }
}
void Chart::drawLinearData(QPainter &painter, QVector<double> &data)
{
    ...
    for(int i=1; i<dataSize; i++)
        painter.drawLine(QLineF(gx+(i-1)*dx, gy+(data[i-1]*dy)+gh/2, gx+dx*i, gy+(data[i]*dy)+gh/2));
}
...
void Chart::drawBarsData(QPainter &painter, QVector<double> &data)
{
 ...
 for(int i=0;i<data.size();i+=128){
   double sum = 0.0;
    ...
    sum/=128;
    painter.drawRect(QRectF(gx+i*dx, gy+gh, 128*dx, (sum*gh)-gh));
 }
}
void Chart::drawSpectrumData(QPainter &painter){
    if(repaint){
        painter.drawImage(QRectF(gx,gy,gw,gh),spect);
        repaint = false;
    }
    //painter.
}
void Chart::fillSpect(int x, std::vector<double> arr){
